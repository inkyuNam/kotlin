...
fun main() {
    ...
    // 코틀린의 배열생성은  arrayOf 를 자주 이용한다.
    val array1 = arrayOf(10,20,30,40,50)
    println("array1 : $array1")     // 클래스 이름과 해쉬코드 등이 출력된다.

    // 배열의 요소들을 한 번에 출력하려면 contentToString 함수를 이용
    println("array1 : ${array1.contentToString()}")

    // 코틀린의 배열은 일단 자료형의 제약이 없어서, 자바의 리스트 등처럼 여러 자료형데이터를 하나의 배열에
    // 넣어서 사용할 수 있다. 다만 그것이 권장할 만한 동작은 아니기 때문에 자료형을 제한하는 코드가 제공되어 사용된다.
    // 자료형 제약없이 사용 가능
    ...
    val array5 = arrayOf<String>("문자열1","문자열2","문자열3","문자열4")  // 객체형 자료는 제네릭을 사용한다.
    ...
    // 다섯 개의 널이 채워진 배열, 코틀린의 변수는 null 을 허용하지 않는 것이 기본이므로
    // 이와 같이 ? 를 넣어야 null 값을 저장할 수 있다.
